...
/**
 * Raytracer — same Blinn-Phong + reflection + refraction logic as the original.
 *
 * OPTIMIZATIONS ADDED (result is pixel-identical to the original):
 *
 *  1. Multi-threading via ForkJoinPool — the image is split into horizontal
 *     bands, each rendered on a separate CPU core. Speedup ≈ N_cores × faster.
 *
 *  2. BVH traversal — if Scene.buildBVH() was called, findClosest() and
 *     isInShadow() use the BVH tree (O(log n)) instead of iterating every
 *     object (O(n)). For scenes with thousands of triangles this is the
 *     single biggest win.
 *
 *  3. Shadow early-exit — the BVH shadow query stops at the first blocker,
 *     matching the original logic but now without scanning the full list.
 *
 * Nothing else changed: shading math, reflection, refraction, tone mapping,
 * bias values, background colour — all identical to the original.
 */
public class Raytracer {
    ...
    // ── ForkJoin task: renders a horizontal band [yStart, yEnd) ──────────────
    ...
    // ── Core recursive ray tracer (unchanged logic) ───────────────────────────
    ...
    // ── Snell's Law refraction vector (unchanged) ─────────────────────────────
    ...
    // ── Closest intersection — uses BVH if available ──────────────────────────
    ...
    // ── Shadow test — uses BVH shadow query if available ─────────────────────
