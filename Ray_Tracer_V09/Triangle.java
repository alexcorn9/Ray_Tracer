...
public class Triangle extends Object3D {
    ...
    private Vector3D interpolateNormal(double u, double v,
        ...
        double w = 1.0 - u - v;
        Vector3D n = aN.multiply(w).add(bN.multiply(u)).add(cN.multiply(v)).normalize();
        return (n.getZ() < 0) ? n.multiply(-1) : n;
    }
    ...
    private Vector3D computeNormal(Vector3D a, Vector3D b, Vector3D c) {
        Vector3D n = b.subtract(a).crossProduct(c.subtract(a)).normalize();
        return (n.getZ() < 0) ? n.multiply(-1) : n;
    }
}